    ...
    <x-filament::section>
        <x-slot name="heading">
            <div class="flex w-full items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ __('ML Scoring System') }}</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('Performance & System Health') }}</p>
                    </div>
                </div>
                @if ($data['mlEnabled'])
                    <x-filament::badge color="success" icon="heroicon-o-check-circle">{{ __('Active') }}</x-filament::badge>
                @else
                    <x-filament::badge color="gray" icon="heroicon-o-x-circle">{{ __('Disabled') }}</x-filament::badge>
                @endif
            </div>
        </x-slot>
        ...
        <div class="mb-8 grid gap-6 md:grid-cols-2">
            <div
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex items-center gap-2 text-xs font-medium text-gray-700 dark:text-gray-300">
                        ...
                        {{ $aucStyles['label'] }} - {{ __('Target') }}: &gt; 0.72
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ __('Last 7 days') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8 grid gap-4 md:grid-cols-4">
            <div
                ...
                <div class="mb-3 flex items-center gap-2">
                    ...
                    <p class="text-xs font-semibold text-gray-600 dark:text-gray-400">{{ __('Model Version') }}</p>
                </div>
                <p class="text-base font-bold text-gray-900 dark:text-white">{{ $data['modelVersion'] }}</p>
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ __('Trained') }}: <span
            ...
            <div
                ...
                <div class="mb-3 flex items-center gap-2">
                    ...
                    <p class="text-xs font-semibold text-gray-600 dark:text-gray-400">{{ __('Training Data') }}</p>
                </div>
                <p class="text-base font-bold text-gray-900 dark:text-white">{{ number_format($data['dataRecords']) }}
                </p>
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ __('records') }}</p>
            </div>
            ...
            <div
                ...
                <div class="mb-3 flex items-center gap-2">
                    ...
                    <p class="text-xs font-semibold text-gray-600 dark:text-gray-400">{{ __('Exploration (epsilon)') }}
                    </p>
                </div>
                <p class="text-base font-bold text-gray-900 dark:text-white">{{ $data['epsilon'] }}</p>
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    @if ($data['elapsedDays'] !== null)
                        <span class="font-medium">{{ __('Day') }} {{ $data['elapsedDays'] }}</span>
                    @else
                        {{ __('Not activated') }}
                    @endif
                </p>
            ...
            <div class="rounded-xl border p-4 transition-shadow hover:shadow-md {{ $circuitStyles['card'] }}">
                <div class="mb-3 flex items-center gap-2">
                    ...
                    <p class="text-xs font-semibold text-gray-600 dark:text-gray-400">{{ __('FastAPI') }}</p>
                </div>
        ...
        <div class="mb-8">
            <div class="mb-4 flex items-center justify-between">
                <div>
                    <p class="text-sm font-bold text-gray-900 dark:text-white">{{ __('Acceptance Rate Trend') }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('Last 7 Days') }}</p>
                </div>
        ...
        <div class="flex items-center justify-between border-t border-gray-200 pt-4 dark:border-gray-700">
            <div>
                <p class="text-xs text-gray-600 dark:text-gray-400">
                    {{ __('Last updated') }}: <span class="font-medium">{{ now()->format('H:i:s') }}</span>
                </p>
            </div>
            <x-filament::button wire:click="retrain" wire:loading.attr="disabled" icon="heroicon-o-arrow-path"
                color="primary" size="sm" class="ml-auto">

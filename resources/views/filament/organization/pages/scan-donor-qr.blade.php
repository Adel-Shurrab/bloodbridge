                ...
                <x-filament::section>
                    <x-slot name="heading">
                        <div style="display: flex; align-items: center; gap: 0.5rem; color: var(--primary-600, #2563EB);" class="dark:text-primary-400">
                            ...
                            {{ __('organization.donor_information') }}
                        </div>
                    </x-slot>
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 1.5rem; padding: 1rem;">
                        ...
                        <div style="text-align: center;">
                            ...
                            <div style="margin-top: 0.5rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                                <span style="border-radius: 9999px; background-color: #FEE2E2; padding: 0.25rem 0.75rem; font-size: 0.875rem; font-weight: 500; color: #B91C1C;" class="dark:bg-red-900/30 dark:text-red-400">
                                    {{ $foundResponse->donor->blood_type?->label() ?? __('organization.not_specified') }}
                                </span>
                        ...
                        <div style="display: grid; width: 100%; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1rem; padding-top: 1rem;">
                            <x-filament::button size="xl" color="success" wire:click="confirmAdmission" style="height: 3.5rem; font-size: 1.125rem;">
                                ...
                                {{ __('organization.confirm_admission') }}
                            </x-filament::button>
                ...
                <x-filament::section>
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
                        ...
                        <div id="scan-status-badge" style="display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; border-radius: 9999px; padding: 0.5rem 1.5rem; font-size: 1rem; font-weight: 600; transition: all 300ms; background-color: #FEF3C7; color: #D97706;">
                            <span id="scan-status-text">{{ __('organization.loading_camera') }}...</span>
                        </div>
                ...
                <x-filament::section>
                    <div style="display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 1rem;">
                        <div style="border-radius: 0.5rem; padding: 1rem; text-align: center;" class="bg-gray-50 dark:bg-gray-800/50">
                            <div style="font-size: 0.75rem; font-weight: 500; color: #6B7280;" class="dark:text-gray-400">{{ __('organization.total_scans_today') }}</div>
                        ...
                        <div style="border-radius: 0.5rem; padding: 1rem; text-align: center;" class="bg-green-50 dark:bg-green-900/20">
                            <div style="font-size: 0.75rem; font-weight: 500; color: #16A34A;" class="dark:text-green-400">{{ __('organization.successful_verifications') }}</div>
                        ...
                        <div style="border-radius: 0.5rem; padding: 1rem; text-align: center;" class="bg-red-50 dark:bg-red-900/20">
                            <div style="font-size: 0.75rem; font-weight: 500; color: #DC2626;" class="dark:text-red-400">{{ __('organization.failed_verifications') }}</div>
            ...
            <x-filament::section>
                <x-slot name="heading">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <span>{{ __('organization.instructions_for_use') }}</span>
                    </div>
                </x-slot>

                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    @foreach([
                        '1' => __('organization.allow_browser_camera_access'),
                        '2' => __('organization.point_camera_at_qr'),
                        '3' => __('organization.wait_for_automatic_verification'),
                        '4' => __('organization.ensure_sufficient_lighting')
                    ] as $step => $text)
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                async function startScanning() {
                    try {
                        ...
                        if (!devices || devices.length === 0) {
                            updateStatus('❌ {{ __('organization.no_camera_found') }}', 'error');
                            return;
                        }
                        ...
                        updateStatus('📷 {{ __('organization.camera_ready_start_scanning') }}', 'ready');

                    } catch (err) {
                        console.error('Error starting scanner:', err);
                        updateStatus('❌ {{ __('organization.failed_to_start_camera') }} ' + err, 'error');
                    }
                }
                ...
                function onScanSuccess(decodedText, decodedResult) {
                    ...
                    isProcessing = true;
                    scanCount++;
                    updateStatus('🔄 {{ __('organization.verifying_code') }}', 'loading');
                    updateStats();

                    @this.verifyQRCode(decodedText).then((result) => {
                        if (result === true) {
                            successCount++;
                            updateStatus('✅ {{ __('organization.verified_successfully') }}', 'success');
                            updateStats();
                            ...
                            setTimeout(() => { updateStatus('📷 {{ __('organization.ready_for_next_scan') }}', 'ready'); isProcessing = false; }, 2000);
                        } else {
                            failureCount++;
                            updateStatus('❌ {{ __('organization.invalid_or_already_used_code') }}', 'error');
                            updateStats();
                            setTimeout(() => { updateStatus('📷 {{ __('organization.ready_for_next_scan') }}', 'ready'); isProcessing = false; }, 4000);
                        }
                    }).catch((error) => {
                        console.error(error);
                        isProcessing = false;
                        updateStatus('❌ {{ __('organization.connection_error') }}', 'error');
                    });
                }

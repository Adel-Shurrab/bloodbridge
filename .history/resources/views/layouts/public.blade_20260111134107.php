...
<head>
    ...
    <meta name="description"
        content="{{ $description ?? ($settings->seo_description ?? 'BloodBridge - نظام ذكي يربط المتبرعين بالمحتاجين. تبرع بالدم وأنقذ الأرواح اليوم.') }}" />
    <meta name="keywords"
        content="{{ $settings->seo_keywords ?? 'التبرع بالدم, جسر الدم, نقل الدم, المتبرعين, المستشفيات' }}" />
    <meta name="author" content="{{ $settings->site_name }}" />
    <meta name="theme-color" content="#DC143C" />
    <meta property="og:title" content="{{ $title ?? ($settings->seo_title ?? 'BloodBridge - نظام التبرع بالدم') }}" />
    <meta property="og:description"
        content="{{ $description ?? ($settings->seo_description ?? 'نظام ذكي يربط المتبرعين بالمحتاجين') }}" />
    <meta property="og:type" content="website" />

    <title>{{ $title ?? ($settings->seo_title ?? 'BloodBridge - إنقاذ الأرواح قطرة قطرة') }}</title>

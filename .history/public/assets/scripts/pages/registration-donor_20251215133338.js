...
function initHealthProfileChangeListeners() {
    ...
    // Add special listener for chronic_disease checkbox
    const chronicDiseaseCheckbox = document.getElementById('chronic_disease');
    if (chronicDiseaseCheckbox) {
        chronicDiseaseCheckbox.addEventListener('change', function() {
...
function validateStep(step) {
    ...
    if (step === 1) {
        ...
        // CHECK CHRONIC DISEASE FIRST - BEFORE OTHER VALIDATIONS
        if (chronicDisease) {
            // Show the "Thank You" Modal
            const modal = document.getElementById('ineligibleModal');
            if (modal) {
                modal.style.display = 'flex';
                console.log('Modal shown - chronic disease detected');
            }
        ...
        // Re-enable the next button if chronic disease is not checked
        const nextBtn = document.getElementById('nextBtn');
        if (nextBtn) {
            nextBtn.disabled = false;
            nextBtn.style.opacity = '1';

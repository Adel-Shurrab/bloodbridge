...
class CheckForMaintenanceMode
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($settings->site_active) {
            ...
            $user = $request->user();
            $path = $request->path();
            \Illuminate\Support\Facades\Log::info("Maintenance Check: Path={$path}, User=" . ($user ? $user->email : 'Guest') . ", Role=" . ($user ? $user->role : 'N/A'));
            ...
            if ($user && $user->role === \App\Models\User::ROLE_ADMIN) {
                \Illuminate\Support\Facades\Log::info("Maintenance Check: Bypassing for Admin");
                return $next($request);
            }
            ...
            if (
                ...
                \Illuminate\Support\Facades\Log::info("Maintenance Check: Allowing essential route");
                return $next($request);
            }
            ...
            \Illuminate\Support\Facades\Log::info("Maintenance Check: Blocking access and showing maintenance page");

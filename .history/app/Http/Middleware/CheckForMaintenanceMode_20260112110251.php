...
class CheckForMaintenanceMode
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($settings->site_active) {
            ...
            // 2. If it's a non-admin authenticated user, log them out automatically
            if ($user) {
                \Illuminate\Support\Facades\Auth::logout();
                $request->session()->invalidate();
                $request->session()->regenerateToken();

                // Redirect back to same URL to refresh state
                return redirect($request->fullUrl());
            }

            // 3. Allow essential auth and system routes for everyone (to allow admin login)
            if (
                $request->is('login*') ||
                $request->is('*/login') ||
                $request->is('logout') ||

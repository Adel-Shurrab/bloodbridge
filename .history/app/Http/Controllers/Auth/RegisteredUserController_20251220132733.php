...
class RegisteredUserController extends Controller
{
    ...
    public function storeDonor(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'gender' => ['required', 'in:' . implode(',', constants('gender_list'))],
            ...
            'blood_type' => ['nullable', 'string', 'in:' . implode(',', constants('blood_types_list'))],
        ], [
        ...
        DB::transaction(function () use ($request) {
            ...
            $user = User::create([
                ...
                'role' => app_constant('user.roles.DONOR'),
    ...
    public function storeOrganization(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            if ($request->hasFile('licenseUpload')) {
                // Store in: storage/app/public/licenses
            ...
            $user = User::create([
                ...
                'phone' => $request->contactPhone, // نستخدم رقم المنظمة كرقم للمستخدم مبدئياً
                'password' => Hash::make($request->adminPassword),
                'role' => app_constant('user.roles.ORGANIZATION'),
                'is_active' => true, // يدخل مباشرة
            ]);
            ...
            $organization = \App\Models\Organization::create([
                ...
                // 'type' column is removed as requested
                ...
                'city' => $request->city,
                // We store the full address in one of the existing columns or just rely on city/lat/lng later
                // For now, mapping HTML inputs to DB columns:
        ...
        // Redirect to the Organization Dashboard (Filament Panel)
        return redirect()->to('/org')->with('success', 'تم تقديم طلبك بنجاح! حسابك قيد المراجعة حالياً.');
    }

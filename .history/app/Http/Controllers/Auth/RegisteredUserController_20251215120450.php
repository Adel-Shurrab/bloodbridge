...
class RegisteredUserController extends Controller
{
    ...
    private function checkEligibility($request): array
    {
        ...
        // 1. Check Permanent Disqualifiers (Safety Net)
        // Even if JS is bypassed, we mark them ineligible here
        ...
        if ($request->boolean('infection')) {
            $isEligible = false;
            // Infection usually clears in 14 days, strictly speaking requires doctor clearance
        ...
        if ($request->boolean('recent_donation') && $request->last_donation_date) {
            ...
            if ($daysSinceDonation < 90) {
                ...
                // Logic Fix: Pick the FURTHEST date
